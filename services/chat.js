...
const { param } = require('../routers/completion');
dotenv.config();
...
/**
 * Completion을 위한 Chat객체를 생성합니다.
 *
 * @param {String} menu
 * @param {String} manual
 * @public
 */
function create(menu, manual) {
  //생성자 초기화 로직
//db 접속해서 프롬프트 세팅
  return {
...
//context string 반환
//message 추가
async function request(chat, msg) {
  if (msg !== undefined) {
  ...
  try {
    const completion = await openai.createChatCompletion({
      model: 'gpt-3.5-turbo',
      max_tokens: 300,
      // completion_id : this.completion_log,
...
module.exports = { create, request, orderState };